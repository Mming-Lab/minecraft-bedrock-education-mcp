 ...
 * ツール別の推奨制限値計算
 */
export function getRecommendedLimit(
  toolType: keyof typeof BUILD_LIMITS
): number {
 ...
 * 最適化効果予測に基づく制限チェック
 */
export function estimateOptimizationRatio(
  toolType: keyof typeof BUILD_LIMITS,
  blockCount: number
): number {
  // ツール別の平均圧縮率
  const compressionRatios = {
    ...
    HELIX: 0.2, // 20%削減（線形構造）
    LINE: 0.3, // 30%削減（線形構造）
    ROTATE: 0.7, // 70%削減（平均）
    TRANSFORM: 0.7, // 70%削減（平均）
  };
