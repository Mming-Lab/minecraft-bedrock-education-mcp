...
exports.calculateBezierPositions = calculateBezierPositions;
exports.estimateBezierArcLength = estimateBezierArcLength;
exports.calculateAdaptiveSegments = calculateAdaptiveSegments;
exports.calculateCubicBezier = calculateCubicBezier;
exports.calculateQuadraticBezier = calculateQuadraticBezier;
const coordinate_utils_1 = require("./coordinate-utils");
/**
 * ベジェ曲線の座標を計算（可変制御点対応）
 *
 ...
 * ```
 */
function calculateBezierPositions(startPoint, endPoint, controlPoints, segments = 50) {
    ...
    for (let i = 0; i <= segments; i++) {
        ...
        for (let j = 0; j <= n; j++) {
            const basis = (0, coordinate_utils_1.bernsteinBasis)(j, n, t);
 ...
 * 直線経路を計算
 */
function calculateLinearPath(start, end, segments) {
    const positions = [];
    for (let i = 0; i <= segments; i++) {
        const t = i / segments;
        positions.push({
    ...
    return (0, coordinate_utils_1.removeDuplicatePositions)(positions);
}
/**
 ...
 * @returns 推定弧長
 */
function estimateBezierArcLength(startPoint, endPoint, controlPoints, samplePoints = 100) {
    const allPoints = [startPoint, ...controlPoints, endPoint];
    ...
    for (let i = 0; i <= samplePoints; i++) {
        ...
        if (prevPoint !== null) {
            totalLength += (0, coordinate_utils_1.calculateDistance)(prevPoint.x, prevPoint.y, prevPoint.z, currentPoint.x, currentPoint.y, currentPoint.z);
        }
 ...
 * @returns 推奨セグメント数
 */
function calculateAdaptiveSegments(startPoint, endPoint, controlPoints, blocksPerUnit = 1) {
 ...
 * @returns 座標配列
 */
function calculateCubicBezier(start, end, control1, control2, segments = 50) {
 ...
 * @returns 座標配列
 */
function calculateQuadraticBezier(start, end, control, segments = 50) {
