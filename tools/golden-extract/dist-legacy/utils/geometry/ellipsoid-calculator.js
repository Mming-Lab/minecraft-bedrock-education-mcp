...
exports.calculateEllipsoidPositions = calculateEllipsoidPositions;
/**
 * 楕円体座標を計算
 ...
 */
function calculateEllipsoidPositions(center, radiusX, radiusY, radiusZ, hollow = false) {
    const positions = [];
    ...
    for (let x = center.x - maxRadius; x <= center.x + maxRadius; x++) {
        for (let y = center.y - maxRadius; y <= center.y + maxRadius; y++) {
            for (let z = center.z - maxRadius; z <= center.z + maxRadius; z++) {
                const normalizedDistance = Math.sqrt(Math.pow((x - center.x) / radiusX, 2) +
