...
exports.getCoordinateValidationMessage = exports.getWarningMessage = exports.getInfoMessage = exports.getSuccessMessage = exports.getErrorMessage = exports.COORDINATE_VALIDATION = exports.BUILD_WARNINGS = exports.BUILD_INFO = exports.BUILD_SUCCESS = exports.BUILD_ERRORS = void 0;
exports.createErrorMessage = createErrorMessage;
const locale_manager_1 = require("./locale-manager");
/**
 * エラーメッセージ（英語・日本語）
 */
exports.BUILD_ERRORS = {
    en: {
        // 座標関連
        ...
        RADIUS_TOO_LARGE: (max) => `Radius must not exceed ${max} blocks`,
        RADIUS_OUT_OF_RANGE: (min, max) => `Radius must be between ${min} and ${max}`,
        // サイズ制限
        TOO_MANY_BLOCKS: (max, actual) => `Too many blocks to place (${actual.toLocaleString()} > ${max.toLocaleString()} limit)`,
        ...
        HEIGHT_OUT_OF_RANGE: (min, max) => `Height must be between ${min} and ${max}`,
        ...
        TOO_MANY_CONTROL_POINTS: (max) => `Too many control points (maximum: ${max})`,
        // マテリアル関連
        ...
        // 内部エラー
        UNKNOWN_ACTION: (action) => `Unknown action: ${action}. Only 'build' is supported`,
    ...
    ja: {
        ...
        RADIUS_TOO_LARGE: (max) => `半径は${max}ブロック以下である必要があります`,
        RADIUS_OUT_OF_RANGE: (min, max) => `半径は${min}から${max}の間である必要があります`,
        // サイズ制限
        TOO_MANY_BLOCKS: (max, actual) => `配置するブロック数が多すぎます（${actual.toLocaleString()} > ${max.toLocaleString()}の制限）`,
        BLOCKS_EXCEED_SAFETY_LIMIT: "パフォーマンス上の理由により、形状に必要なブロック数が多すぎます",
        // 形状パラメータ
        INVALID_HEIGHT: "無効な高さ: 正の数である必要があります",
        HEIGHT_OUT_OF_RANGE: (min, max) => `高さは${min}から${max}の間である必要があります`,
        ...
        TOO_MANY_CONTROL_POINTS: (max) => `制御点が多すぎます（最大: ${max}）`,
        // マテリアル関連
        ...
        // 内部エラー
        UNKNOWN_ACTION: (action) => `不明なアクション: ${action}。'build'のみサポートされています`,
 ...
 * 成功メッセージ（英語・日本語）
 */
exports.BUILD_SUCCESS = {
    en: {
        SHAPE_PLACED: (shape, blocks) => `${shape} placed successfully: ${blocks.toLocaleString()} blocks`,
        BUILD_COMPLETE: "Build operation completed",
        OPERATION_QUEUED: "Build operation queued and starting",
    },
    ja: {
        SHAPE_PLACED: (shape, blocks) => `${shape}の配置が完了しました: ${blocks.toLocaleString()}ブロック`,
 ...
 * 警告メッセージ（英語・日本語）
 */
exports.BUILD_WARNINGS = {
    en: {
        LARGE_SHAPE: (blocks) => `Warning: Large shape with ${blocks.toLocaleString()} blocks may take time to build`,
    ...
    ja: {
        LARGE_SHAPE: (blocks) => `警告: ${blocks.toLocaleString()}ブロックの大きな形状は建築に時間がかかる場合があります`,
 ...
 * 座標検証メッセージ（英語・日本語）
 */
exports.COORDINATE_VALIDATION = {
    en: {
        X_OUT_OF_RANGE: (x) => `X coordinate ${x} is out of valid range`,
        Y_OUT_OF_RANGE: (y) => `Y coordinate ${y} is out of valid range (-64 to 320)`,
        Z_OUT_OF_RANGE: (z) => `Z coordinate ${z} is out of valid range`,
        NORMALIZED_TO: (original, normalized) => `Coordinate normalized from ${original} to ${normalized}`,
    },
    ja: {
        X_OUT_OF_RANGE: (x) => `X座標 ${x} が有効範囲外です`,
        Y_OUT_OF_RANGE: (y) => `Y座標 ${y} が有効範囲外です（-64～320）`,
        Z_OUT_OF_RANGE: (z) => `Z座標 ${z} が有効範囲外です`,
        NORMALIZED_TO: (original, normalized) => `座標が ${original} から ${normalized} に正規化されました`,
    },
};
/**
 * エラーレスポンスを生成するヘルパー関数（多言語対応）
 */
function createErrorMessage(type, messageKey, ...args) {
    ...
    const prefix = (0, locale_manager_1.t)(prefixMessages, type);
    return `${prefix} ${messageKey}`;
}
/**
 * 便利な翻訳関数群
 */
const getErrorMessage = (key, ...args) => (0, locale_manager_1.t)(exports.BUILD_ERRORS, key, ...args);
exports.getErrorMessage = getErrorMessage;
const getSuccessMessage = (key, ...args) => (0, locale_manager_1.t)(exports.BUILD_SUCCESS, key, ...args);
exports.getSuccessMessage = getSuccessMessage;
const getInfoMessage = (key, ...args) => (0, locale_manager_1.t)(exports.BUILD_INFO, key, ...args);
exports.getInfoMessage = getInfoMessage;
const getWarningMessage = (key, ...args) => (0, locale_manager_1.t)(exports.BUILD_WARNINGS, key, ...args);
exports.getWarningMessage = getWarningMessage;
const getCoordinateValidationMessage = (key, ...args) => (0, locale_manager_1.t)(exports.COORDINATE_VALIDATION, key, ...args);
exports.getCoordinateValidationMessage = getCoordinateValidationMessage;
