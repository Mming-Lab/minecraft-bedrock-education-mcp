...
exports.rotatePoint3D = rotatePoint3D;
exports.rotatePointsArray = rotatePointsArray;
/**
 ...
 * @returns 回転後の座標
 */
function rotatePoint3D(point, origin, axis, angleDegrees) {
    // 回転軸を表す単位ベクトル
    const n = [0, 0, 0];
    if (axis === "x") {
        n[0] = 1;
 ...
 * @returns 回転後の座標配列
 */
function rotatePointsArray(points, origin, axis, angleDegrees) {
    return points.map((point) => rotatePoint3D(point, origin, axis, angleDegrees));
}
